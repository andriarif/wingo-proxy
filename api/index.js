export default async function handler(req, res) {
  ...
  try {
    ...
    const issueRes = await fetch(`${BASE}/GetGameIssue`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        typeId: 30,
    ...
    const resultRes = await fetch(`${BASE}/GetNoaverageEmerdList`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        pageSize: 10,
    ...
    res.status(200).json({
      ...
      resultPeriode: last?.issueNumber || null,
      angka: last ? parseInt(last.number) : null
    });
